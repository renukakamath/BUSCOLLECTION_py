...
public class AddExpenses extends AppCompatActivity implements JsonResponse {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        JR.json_response = (JsonResponse) AddExpenses.this;
        String q = "/Viewexp?login_id=" + sh.getString("log_id", "") + "&bid=" + ViewBus.bid;
        q = q.replace(" ", "%20");
        JR.execute(q);
        ...
        b1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                JsonReq JR = new JsonReq();
                JR.json_response = (JsonResponse) AddExpenses.this;
                String q = "/Addexp?exp=" + exp + "&login_id=" + sh.getString("log_id", "") + "&bid=" + ViewBus.bid + "&amount=" + amount;
                q = q.replace(" ", "%20");
                JR.execute(q);
            ...
            if (method.equalsIgnoreCase("AddCol")) {
                ...
                if (status.equalsIgnoreCase("success")) {
                    ...
                    for (int i = 0; i < ja1.length(); i++) {
                        ...
                        expence_name[i] = ja1.getJSONObject(i).getString("expence_name");
                        expence_amount[i] = ja1.getJSONObject(i).getString("expence_amount");
                        date[i] = ja1.getJSONObject(i).getString("date");


                        value[i] = "employee_name: " + employee_name[i] + "\nbus_number: " + bus_number[i] + "\nexpence_name: " + expence_name[i] + "\nexpence_amount: " + expence_amount[i] + "\ndate:" + date[i];
